...
void second_color_left(int first) {
  int T90 = EEPROM_read_int(T90_address);
  ...
  cnt = 0;
  while (cnt / 100 < T90 / 3) {
    if (color_L == GREEN && first != GREEN) {
      item = GREEN;
      beep(2);
      break;
    } else if (color_L == RED && first != RED) {
      item = RED;
      beep(2);
      break;
    } else if (color_L == SILVER && first != SILVER) {
      item = SILVER;
      beep(2);
      break;
    } else if (color_L == YELLOW && first != YELLOW) {
      item = YELLOW;
      beep(2);
      break;
    } else if (color_L == BLUE && first != BLUE) {
      item = BLUE;
      beep(2);
      break;
    } else item = WHITE;
  ...
  if (item == GREEN && first != GREEN) {
  ...
  } else if (item == SILVER && first != SILVER) {
    lcd.clear();
    lcd.print("obstacle");
    obstacle(first);
  } else if (item == YELLOW && first != YELLOW) {
    lcd.clear();
    lcd.print("archery");
    archery(first);
  } else if (item == BLUE && first != BLUE) {
    lcd.clear();
    lcd.print("wlift");
    wlift(first);
    ...
    lcd.clear();
    lcd.print("no inter or no color");
  }
  if (item != WHITE && item != RED) {
    go(50, 50);
    move(forward, T90 / 2.5, 150);
    move(Left, T90, 150);
    move(forward, T90 / 3, 150);
  }
...
void second_color_right(int first) {
  int T90 = EEPROM_read_int(T90_address);
  int inter_detected = false;
  int item = WHITE;
  if (SR1 && SM) {
  ...
  cnt = 0;
  while (cnt / 100 < T90 / 2.5) {
    if (color_R == GREEN && first != GREEN) {
      item = GREEN;
      beep(2);
      break;
    } else if (color_R == RED && first != RED) {
      item = RED;
      beep(2);
      break;
    } else if (color_R == SILVER && first != SILVER) {
      item = SILVER;
      beep(2);
      break;
    } else if (color_R == YELLOW && first != YELLOW) {
      item = YELLOW;
      beep(2);
      break;
    } else if (color_R == BLUE && first != BLUE) {
      item = BLUE;
      beep(2);
      break;
    } else item = WHITE;
